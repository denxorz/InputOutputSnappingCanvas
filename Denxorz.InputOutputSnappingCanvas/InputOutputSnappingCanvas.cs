...
public class InputOutputSnappingCanvas : DragCanvas
{
    ...
    public void ForceLinkAll()
    {
        ...
        InputOutputSnappingCanvas.RemoveDeadLinks(allInputs, allOutputs);
    }
    ...
    private void UnSetSnap()
    {
        ...
        if (ElementBeingDragged is not ISnapHost)
        {
            return;
        }

        var host = (ISnapHost)ElementBeingDragged;
        foreach (IConnectionInput input in host.Inputs)
        {
    ...
    private static void RemoveDeadLinks(IReadOnlyCollection<IConnectionInput> allInputs, IReadOnlyCollection<IConnectionOutput> allOutputs)
    {
        var deadInputLinks = allInputs
            .Where(input => input.ConnectedOutput != null && !allOutputs.Contains(input.ConnectedOutput))
            .ToList();

        foreach (var dead in deadInputLinks.Where(d => d?.ConnectedOutput is not null))
        {
        ...
        foreach (var dead in deadOutputLinks.Where(d => d?.ConnectedInput is not null))
        {
